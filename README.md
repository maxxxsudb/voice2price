...
├── UI_CONTRAST_REQUIREMENTS.md  # ⚠️ ОБЯЗАТЕЛЬНЫЕ требования контрастности UI
└── README.md
```

## Требования к UI (обязательно)

**Все подписи, введенные тексты, плейсхолдеры и любые элементы интерфейса должны проверяться на контрастность с фоном** — как в неизмененном состоянии, так и после изменений (валидация, disabled, focus). Черный текст на темном фоне (например, плохо видимая подпись «API_KEY — API-ключ сервисного аккаунта SpeechKit *») — критический дефект. Полные правила: [UI_CONTRAST_REQUIREMENTS.md](UI_CONTRAST_REQUIREMENTS.md).
