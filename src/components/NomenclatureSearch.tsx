    ...
    for (const result of results) {
      ...
      for (const term of terms) {
        ...
        while (true) {
          ...
          const context = result.text.substring(ctxStart, ctxEnd);
              ...
              {matches.map((match, idx) => (
                <div key={idx} className="bg-black/20 rounded-xl p-4">
                  ...
                  <p className="text-gray-200 text-sm">
                    ...{highlightTerm(match.context, match.term)}...
                  </p>
                </div>
              ))}
            </div>
      ...
      {results.length === 0 && (
        <div className="bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 p-12 text-center">
          ...
          <p className="text-gray-500 text-sm mt-1">
            Добавьте термины номенклатуры и загрузите аудиофайлы
          </p>
        </div>
      )}
    </div>
  ...
  return (
    <>
      ...
      <mark className="bg-yellow-400/30 text-yellow-200 px-0.5 rounded">
        {text.substring(pos, pos + term.length)}
      </mark>
      {text.substring(pos + term.length)}
    </>
  );
}
