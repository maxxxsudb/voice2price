    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-500/10 border border-blue-500/20 rounded-2xl p-6">
        ...
        <ol className="text-blue-200/80 text-sm space-y-1 list-decimal list-inside">
          ...
          <li>Или получите IAM-токен через CLI: <code className="bg-blue-500/20 px-1 rounded">yc iam create-token</code></li>
        </ol>
      </div>
      ...
      <div className="bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 p-6 space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            >
              ...
              <option value="general:rc" className="bg-slate-800">Общая RC (general:rc)</option>
              <option value="maps" className="bg-slate-800">Топонимы (maps)</option>
              <option value="dates" className="bg-slate-800">Даты и числа (dates)</option>
              <option value="names" className="bg-slate-800">Имена (names)</option>
              <option value="numbers" className="bg-slate-800">Числа (numbers)</option>
            </select>
          </div>
        </div>
        ...
        <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4">
          <p className="text-yellow-300/80 text-xs flex items-start gap-2">
            ...
            <span>
              <strong>Важно:</strong> Прямые запросы из браузера к SpeechKit API могут блокироваться CORS. 
              Для надёжной работы используйте Python скрипт (вкладка "Python скрипт") или настройте прокси-сервер.
              Ключ хранится только в вашем браузере и никуда не отправляется кроме API Яндекса.
            </span>
          </p>
        </div>
      </div>
