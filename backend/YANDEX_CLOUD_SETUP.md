...
Пример JSON-ключа (правильный формат для Яндекс Облака):
```json
{
  "id": "ajeg0g3avtndouop26f7",
...
**Важно:** Для работы необходимы поля:
- `id` - идентификатор ключа
- `service_account_id` - идентификатор сервисного аккаунта (используется в JWT как `iss`)
- `private_key` - приватный ключ для подписи JWT

Публичный ключ не используется при генерации токена - он нужен только серверу Яндекса для проверки подписи.
...
### Ошибка: "В JSON-ключе отсутствуют обязательные поля: id, service_account_id или private_key"
Убедитесь, что вы загрузили полный JSON-файл сервисного аккаунта Яндекс Облака.  
**Необходимые поля:**
- `id` - идентификатор ключа (используется в JWT заголовке как `kid`)
- `service_account_id` - идентификатор сервисного аккаунта (используется в JWT как `iss`)
- `private_key` - приватный ключ для подписи JWT

**Как получить правильный JSON-ключ:**
1. В консоли Яндекс Облака перейдите в раздел "Сервисные аккаунты"
2. Выберите нужный сервисный аккаунт
3. Нажмите "Создать новый ключ" → "Создать ключ" → "JSON"
4. Скачайте файл (обычно называется `key.json`)
5. Откройте файл и скопируйте всё содержимое целиком

**Пример правильного JSON-ключа:**
```json
{
  "id": "ajeg0g3avtndouop26f7",
  "service_account_id": "aje...",
...
### Ошибка: "relation 'yandex_cloud_settings' does not exist"
Таблица настроек не создана в базе данных. При первом запуске приложение автоматически создаст все необходимые таблицы. Дождитесь сообщения в логе:
```
✅ Инициализация завершена
```

Если ошибка сохраняется - перезапустите контейнер бэкенда:
```bash
docker compose restart audio-analyzer-backend
```

### Ошибка: "Невалидный JSON"
JSON-ключ должен быть валидным JSON объектом. Проверьте:
- Нет ли обрывов строки
- Все ли кавычки закрыты
- Скопировано ли содержимое файла полностью
...
4. Выберите `editor` или `ai.languageModels.user`
...
Поле с приватным ключом намеренно размывается для безопасности. Нажмите кнопку "👁️ Показать" чтобы увидеть содержимое.
